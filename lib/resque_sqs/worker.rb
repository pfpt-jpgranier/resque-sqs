...
module ResqueSqs
  ...
  class Worker
    include ResqueSqs::Helpers
    extend ResqueSqs::Helpers
    ...
    def self.all_workers_with_expired_heartbeats
      ...
      heartbeats.select do |id, heartbeat|
        if heartbeat
          ...
          seconds_since_heartbeat > ResqueSqs.prune_interval
        else
          false
        end
    ...
    def start_heartbeat
      ...
      @heartbeat_thread_signal = ResqueSqs::ThreadSignal.new

      @heartbeat_thread = Thread.new do
        loop do
          heartbeat!
          signaled = @heartbeat_thread_signal.wait_for_signal(ResqueSqs.heartbeat_interval)
          break if signaled
        end
      end
    ...
    def prune_dead_workers
      return unless data_store.acquire_pruning_dead_worker_lock(self, ResqueSqs.heartbeat_interval)
      ...
      all_workers_with_expired_heartbeats.each do |worker|
        ...
        # client library or an older version of ResqueSqs. We won't touch these.
    ...
    def procline(string)
      $0 = "#{ENV['RESQUE_PROCLINE_PREFIX']}resque-#{ResqueSqs::VERSION}: #{string}"
    ...
    def verbose=(value);
      if value && !very_verbose
        ...
        ResqueSqs.logger.level = Logger::INFO
      elsif !value

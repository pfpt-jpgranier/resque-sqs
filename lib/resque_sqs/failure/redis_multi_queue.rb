  ...
  module Failure
    ...
    class RedisMultiQueue < Base
      ...
      def self.requeue(id, queue = :failed)
        ...
        data_store.update_item_in_failed_queue(id,ResqueSqs.encode(item),queue)
      ...
      def self.requeue_all
        queues.each { |queue| requeue_queue(ResqueSqs::Failure.job_queue_name(queue)) }
      end

      def self.remove_queue(queue)
        data_store.remove_failed_queue(ResqueSqs::Failure.failure_queue_name(queue))
      end
